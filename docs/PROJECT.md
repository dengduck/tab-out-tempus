...
Tab Out 是一个 Chrome 新标签页扩展，用域名分组的方式展示用户所有打开的标签页。内置关闭动画、重复标签检测、"稍后保存"、历史统计等功能。所有数据完全存储在本地（`chrome.storage.local`），无服务器、无账号、无外部 API 调用。
